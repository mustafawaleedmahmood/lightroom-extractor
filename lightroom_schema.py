...
# FIX: Exposure2012 now a proper float (was "0.00" string which caused coercion issues).
# SharpenRadius / PerspectiveRotate / PerspectiveX / PerspectiveY stay as
# format-specific strings — the XMP builder preserves these verbatim.
DEFAULT_SETTINGS = {
    ...
    "SharpenRadius":              "+1.0",   # must stay "+1.0" string format
    "SharpenDetail":              25,
    ...
    "PerspectiveRotate":          "0.0",    # must stay "0.0" string format
    "PerspectiveScale":           100,
    "PerspectiveAspect":          0,
    "PerspectiveUpright":         0,
    "PerspectiveX":               "0.00",   # must stay "0.00" string format
    "PerspectiveY":               "0.00",   # must stay "0.00" string format
    "AutoLateralCA":              0,
    "Exposure2012":               0.0,      # FIX: was "0.00" — now proper float
    "Contrast2012":               0,
    "Highlights2012":             0,
    "Shadows2012":                0,
    "Whites2012":                 0,
...
# Keys whose string format must be preserved exactly as-is in XMP output
STRING_FORMAT_KEYS = {"SharpenRadius", "PerspectiveRotate", "PerspectiveX", "PerspectiveY"}
...
def _coerce_number(value):
    """
    Coerce a value to int/float when possible.
    String-format keys (SharpenRadius, Perspective*) are handled by the
    builder directly, so they are allowed to pass through as floats here.
    """
    if _is_number(value):
        return value
...
def _normalize_curve_points(points):
    ...
    for pt in normalized:
        xv, yv = pt.split(",", 1)
        by_x[int(xv.strip())] = int(yv.strip())

    ordered = [f"{x}, {by_x[x]}" for x in sorted(by_x)]
    if not ordered[0].startswith("0,"):
...
def _apply_raw_crs_settings(source, display, detected_fields):
    ...
    for raw_key, value in raw_settings.items():
        ...
        if not display_key:
            continue
        section = next(
            (sn for sn, fields in SECTION_FIELDS.items() if display_key in fields),
            None,
        )
        if section is None:
            continue
...
def _normalize_curves(source, warnings):
    ...
    raw_curves = source.get("curves") if isinstance(source.get("curves"), dict) else {}
    ...
    for display_key, crs_key in CURVE_MAP.items():
        ...
        points = _normalize_curve_points(value) if value is not None else []
        if points:
            curves[crs_key] = points
        elif value not in (None, "", "custom", "Custom"):
            warnings.append(f"Curve {display_key} was present but had no usable points.")
...
def normalize_preset_payload(payload):
    """
    Normalize any accepted input shape into one stable internal contract.

    Accepted shapes:
      - Display shape: {"Light": {...}, "Effects": {...}, "HSL": {...}, "Curves": {...}}
      - Canonical shape: {"meta": {...}, "settings": {...}, "curves": {...}}
      - Mixed / nested
    ...
    # 1. Copy display-section dicts (Light / Effects / HSL)
    for section_name in SECTION_FIELDS:
        _copy_display_section(source, display, section_name, detected_fields)

    # 2. Apply raw crs: settings if present
    _apply_raw_crs_settings(source, display, detected_fields)

    # 3. Build canonical settings from display values
    settings = _canonical_settings_from_display(display)

    # 4. Overlay any explicit raw settings (canonical.settings or root-level crs keys)
    raw_settings_src = source.get("settings") if isinstance(source.get("settings"), dict) else {}
    for raw_key, value in raw_settings_src.items():
        key = _strip_crs_prefix(raw_key)
    ...
    meta_src = source.get("meta") if isinstance(source.get("meta"), dict) else {}
    for key in meta:
        if key in meta_src and meta_src[key] not in (None, ""):
            meta[key] = str(meta_src[key])

    return {
        "display":         display,
        "canonical":       {"meta": meta, "settings": settings, "curves": curves},
